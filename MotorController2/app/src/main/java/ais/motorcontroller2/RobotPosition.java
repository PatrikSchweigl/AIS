...
public class RobotPosition {
    ...
    public RobotPosition add(RobotPosition other){
        this.x += other.x;
        this.y += other.y;
    ...
    public RobotPosition addXY(RobotPosition other){
        this.x += other.x;
        this.y += other.y;
        return this;
    }

    public float angleBetween(RobotPosition other){
        return (float) Math.toDegrees(Math.atan2(other.x - x, other.y - y));
    }

    public boolean equalsXY(RobotPosition other){
        return x == other.x && y == other.y;
    }

    public boolean equals(RobotPosition other){
        return equalsXY(other) && angle == other.angle;
    }

    public boolean isAt(RobotPosition other){
        int offset = STANDARD_POSITION_OFFSET;
        return isAt(other, offset);
    }

    public boolean isAt(RobotPosition other, int offset){
        return (x <= (other.x + offset)) && (x >= (other.x - offset))
                && (y <= (other.y + offset)) && (y >= (other.y - offset));
    }

    public boolean isAtWithAngle(RobotPosition other){
        int offset = STANDARD_POSITION_OFFSET;
        int angleOffset = STANDARD_ANGLE_OFFSET;
        return isAtWithAngle(other, offset, angleOffset);
    }

    public boolean isAtWithAngle(RobotPosition other, int offset, int angleOffset){
        return      (x <= (other.x + offset)) && (x >= (other.x - offset))
                &&  (y <= (other.y + offset)) && (y >= (other.y - offset))
    ...
    public double dist2goal(RobotPosition other){
        return Math.sqrt((other.x - this.x)*(other.x - this.x)+(other.y - this.y)*(other.y - this.y));
    }

    public float angle2goal(RobotPosition other){
        double alpha = 0;
    ...
    public double distance2mline(RobotPosition goal){
